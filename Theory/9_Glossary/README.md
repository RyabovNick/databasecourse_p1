...
[Подробнее о соединениях таблиц](../5_Queries/README.md)
...
Оператор, который соединяет несколько таблиц, указанных в FROM. Включает: [inner](#markdown-header-inner-join), [left](#markdown-header-left-join), [right](#markdown-header-right-join), [outer](#markdown-header-outer-join).
...
Общее количество чисел в числе с плавающей запятой. Например: NUMBER(5,3) - 5 precision. Сколько всего чисел. 3 - [scale](#markdown-header-scale) - сколько из этиэ 5 чисел после запятой.
...
[План выполнения](#markdown-header-execution-plan), который используется для данного запроса.

## Relation

Отношение - набор [кортежей](#markdown-header-tuple) (tuples)
...
Транзакця - содержит один или несколько операторов SQL. Все операторы в транзакции фиксируются или откатываются вместе. Т.е. либо транзакция выполняется полностью, либо не выполняется совсем.
